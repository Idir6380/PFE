...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=int,
        default=None, 
        help="Number of validation intervals to wait before stopping if no improvement."
        )
    parser.add_argument(
        "--early_stopping_metric", 
        type=str, 
        default="train_loss", 
        help="Metric to monitor for early stopping (e.g., 'train_loss')."
        )
    parser.add_argument(
        "--early_stopping_mode", 
        type=str, 
...
def main(args):
    ...
    # Preprocessing the datasets.
    train_resize = transforms.Resize((512, 384), interpolation=interpolation)  # Use dynamic interpolation method
    train_crop = lambda x: x
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        if accelerator.is_main_process and args.early_stopping_patience is not None:
            ...
            if improved:
                ...
                accelerator.save_state(save_path)
                logger.info(f"Saved best model in epoch {epoch} to {save_path}")
            else:
                epochs_without_improvement += 1
                logger.info(f"No improvement in {args.early_stopping_metric}. Patience: {epochs_without_improvement}/{args.early_stopping_patience}")
